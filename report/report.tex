...
An example on how a unary method that can receive unary messages would be defined can be given using the Duck example:
...
A method name is followed by brackets containing a list of expressions that will be executed in order, separated by dots. This method tells the Duck to increment it's age field by 1.

Babble methods are translated into Java methods belonging to their corresponding class, Sent messages are converted into method calls.
...
An example on how a keyword method that can receive keyword messages would be defined can again be given using the Duck example:
...
The syntax of these methods is very like a unary method, the main difference in syntax is the colon at the end of the method name, followed by a name to which this argument can be referred inside the method. Multiple arguments would be defined using the following syntax:
...
Keyword methods can be used when a method needs to provide functionality based on input.
Keyword methods are also translated into Java methods belonging to their corresponding class, yet just as keyword messages require arguments in Babble, they also take arguments of the stack in the Java bytecode.
...
Just as keyword methods, custom infix methods can be defined by the user using a slightly different syntax:
\begin{quote}
\begin{lstlisting}
==: that [ that class name == "True". ].
\end{lstlisting}
\end{quote}

This syntax is almost identical to a single-argument keyword method, the difference being that instead of starting with a (alphabetical) name, an operator is given.

%TODO infix method declaration.
