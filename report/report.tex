...
From the beginning we knew that Babble would not have a special case for either \texttt{if} or \texttt{while} statements. This meant that we needed Babble to be powerful enough to program our own \texttt{if} and \texttt{while} statements inside Babble. For \texttt{if} this was easy, just check the condition once and act on that. For \texttt{while} this proved to be more troublesome. \texttt{While} needed to be a recursive function on a Block, and due to the way the closure works on blocks, we could not just reassign a number (which is typically used in a \texttt{while} condition (\verb|i < 10|)). To solve the issue of 2 closures having to be able to get and set each other's variables, we eventually created a \texttt{while} loop that makes use of get and set methods in the same class as the method in which the \texttt{while} loop is needed. While this does work, and does create a valid \texttt{while} loop, it is far from desirable. Our conclusion to this is: \begin{quote}Avoid having to use \texttt{while} loops in Babble, resort instead to using recursion. \end{quote}
